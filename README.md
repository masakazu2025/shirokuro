...
社内で結合テストの評価業務を効率化するために作成したツールを、ポートフォリオ用に切り出し・再構築したものです。元は Python・Flask(Jinja2) で実装していましたが、フロントエンドとバックエンドが密結合になりテスト容易性が下がったため、責務を分離した FastAPI + React 構成に作り替えました。実データや社内固有の構造は含まれておらず、細部はデモ用のダミーデータ・簡略化した構造で構成しています。元になったツールの概要は[現行ツール概要](docs/現行ツール概要.md)を参照してください。
